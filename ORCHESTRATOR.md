...
| 2026-06-13 | Updated `_pages/about.md` against CV | Bumped "20+ years" → "30+ years" (career started Aug 1994); added current-role sentence (Infra Engineer @ Rail Delivery Group); linked the 60 Seconds post (`/blog/60-seconds-with-luke-leigh/`) and emphasised GitHub as the data/reference hub. Ref links [2]–[3] with `target="_blank"`. Note: Leigh Services deliberately NOT mentioned — it was a brief contracting vehicle, not an active company; do not frame Luke as a business owner. |
...
| 2026-06-13 | Added "Copy-and-Paste Examples for Real Life" section to the A.I. post | Per Luke's request, six ready-to-use home/office prompts (complaint email, meal plan + shopping list, explain-to-a-child, decode a letter/bill, cover letter, product comparison) in `text` code blocks with `[bracketed]` swap-in context, so non-technical family readers can use the principles directly rather than just reading the theory. |
| 2026-06-13 | Fixed publish-visibility issue on the A.I. post | Post was committed and pushed and the Pages build had succeeded, but the post date (`2026-06-13 00:30:00 +0000`) was ahead of the last build (ran `2026-06-12 23:45 UTC`), so Jekyll's `future: false` correctly excluded it. BST/UTC offset masked the cause — local clock read the 13th while GitHub built on the 12th. Resolved by setting `date`/`last_modified_at` to `2026-06-12 23:00:00 +0000` (already past in UTC, still renders as 13 June in GB timezone), commit `9b90064`, pushed; rebuild completed clean and post verified live. Lesson: date same-day posts at `01:00:00 +0000` or later, and never create a post timestamped close to midnight UTC — it can fall ahead of the triggering build. |
...
| `2026-06-13-the-blinking-cursor-problem.md` | 2026-06-13 | The Blinking Cursor Problem — Getting More Out of A.I. | Published |
